    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          "https://api.weatherbit.io/v2.0/forecast/daily?city=ElMonte%2CCA&postal_code=91733&country=US&days=16&units=I&key=e0adc0a33ed84d1d8986b40f351a8d7e"
        );
    ...
    <div>
      <div className="data-list">
        ...
        <div className="column">
          ...
          {filteredData.map((day, index) => (
            <div>
              <Link key={index} to={`/info/${day.datetime}`}>
                🔗
              </Link>
            </div>
          ))}
      ...
      <div className="buttons-container">
        ...
        <button onClick={handleSortByWindLowToHigh}>
          Wind Strength Filter
        </button>
      </div>
    </div>
  );
